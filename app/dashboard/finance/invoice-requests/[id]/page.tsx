...
export default function InvoiceRequestDetailPage() {
  ...
  const generateSalesInvoice = () => {
    ...
    alert('Sales invoice generation flow will be implemented next.')
  }
            ...
            <CardHeader>
              <div className="flex items-center justify-between gap-3 flex-wrap">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button onClick={() => generateSalesInvoice()} disabled={selectedLines.length === 0}>
                    Generate Sales Invoice
                  </Button>
                </div>
              </div>
              <CardDescription>{data.do_number ?? data.id}</CardDescription>
            </CardHeader>
            <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
              <p><span className="font-medium">Dispatch Order:</span> {data.do_number ?? data.id.slice(0, 8)}</p>
              ...
              <p>
                ...
                <Link className="hover:underline" href={`/dashboard/sales/${data.sales_order_id}`}>
                  {data.sales_order_number ?? data.sales_order_id.slice(0, 8)}
                </Link>
              </p>
