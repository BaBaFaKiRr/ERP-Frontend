...
type LineDetail = { id: string; name: string }
...
export default function RequestMaterialPage() {
  ...
  const setMaterialOpening = (groupIdx: number, rowIdx: number, value: string) => {
    setMaterialGroups((prev) =>
      prev.map((g, gi) =>
        gi !== groupIdx
          ? g
          : { ...g, rows: g.rows.map((r, ri) => (ri === rowIdx ? { ...r, opening_balance: value } : r)) },
      ),
    )
  }
  ...
  const addProduce = () => {
    ...
    if (q + used > picked.maxQty) return alert(`Qty exceeds task bound (${picked.maxQty}) for this item`)
    setProduceEntries((prev) =>
      existing
        ? prev.map((x) => (x.item_id === produceItemId ? { ...x, qty: x.qty + q } : x))
        : [...prev, { item_id: produceItemId, sku: picked.sku, name: picked.name, qty: q, maxQty: picked.maxQty }],
    )
    setProduceQty('')
  }

  const removeProduce = (itemId: string) => setProduceEntries((prev) => prev.filter((x) => x.item_id !== itemId))
  ...
  return (
    <div className="p-6 md:p-8 space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Request Material · {line?.name ?? 'Line'}</CardTitle>
          <CardDescription>Create material request directly from line-level material at hand widget flow.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <div className="rounded-md border p-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-[1fr_140px_100px] gap-2 items-end mb-3">
              <div>
                ...
                <select className="h-10 rounded-md border px-3 bg-background text-sm w-full" value={produceItemId} onChange={(e) => setProduceItemId(e.target.value)}>
                  <option value="">Select item</option>
                  {taskItems.map((x) => (
                    <option key={x.item_id} value={x.item_id}>{x.sku} - {x.name}</option>
                  ))}
                </select>
              </div>
              <div>
              ...
              <Button type="button" onClick={addProduce}>Add</Button>
            </div>
              ...
              <div className="mb-3 space-y-1">
                {produceEntries.map((p) => (
                  <div key={p.item_id} className="text-sm flex items-center justify-between rounded border px-2 py-1">
                    ...
                    <Button type="button" variant="ghost" size="sm" onClick={() => removeProduce(p.item_id)}>Remove</Button>
                  </div>
                ))}
              </div>
            )}
                ...
                materialGroups.map((g, gi) => (
                  <div key={g.product.item_id} className="rounded-md border p-2">
                    ...
                    <div className="hidden md:grid md:grid-cols-[1fr_120px_140px_160px] gap-2 px-1 text-xs font-medium text-muted-foreground">
                      <p>Item</p><p>Total Qty</p><p>Opening Balance</p><p>Material to request</p>
                    </div>
          ...
          <div className="flex justify-end">
            <Button type="button" onClick={() => void requestMaterial()} disabled={requesting || materialGroups.length === 0}>
